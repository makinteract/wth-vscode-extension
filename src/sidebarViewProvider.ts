...
import { getBetterExplanation } from './controller';
...
class SideViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'wth.view';
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(
      async ({ message }: { message: string }) => {
        switch (message) {
          case 'tryagain':
            const explanation = await getBetterExplanation();
            this.sendMessage({ message: explanation });
            break;
